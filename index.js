...
app.post("/login", parseForm, async (req, res) => {
	...
	try {
		// try: checks if username has match in db
		...
		if (isUserValid) {
			...
			req.session.user = {
				// the user object is being created
...
app.get("/profile/viewMyEvents", async (req, res) => {
	...
	try {
		...
		const formatParticipantEventCards = await events.formatParticipantEventCards(
...
// --- SIGN UP FOR TASK
app.post(
	"/profile/browseEvents/:eventID(\\d+)/:eventName",
	parseForm,
	async (req, res) => {
		...
		try {
			...
			res.redirect(
				`/profile/browseEvents/${req.params.eventID}/${req.params.eventName}`
			);
		} catch (err) {
			console.log(err);
		}
...
app.get("/logout", (req, res) => {
	req.session.destroy(() => {
		console.log("The session is now destroyed!!!"); // Gets rid of the user's session!
		res.redirect("/?msg=userLogout"); // Then redirects them to the home page.
	});
});
